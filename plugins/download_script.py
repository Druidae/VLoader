...
from plugins.functions.display_progress import convert_to_human_bytes, progress_for_pyrogram
from plugins.functions.rand_text import random_char
from plugins.keyboard import KeyboardAndText
from plugins.utitles import video_metadata, vm_video_metadata, audio_metadata
...
async def youtube_dl_call_back(_bot, update):
    AD_STRING_TO_REPLACE = ("Please report this issue on https://github.com/Druidae/VLoader/issues/new?labels=bug"
                            "&template=bug-report---.")
    ...
    save_ytdl_json_path = (
        Config.DOWNLOAD_LOCATION + "/" + str(update.from_user.id) + f"{ran}" + ".json"
    )
    ...
    await update.message.edit_caption(
        caption=KeyboardAndText.DOWNLOAD_START.format(custom_file_name)
    )
    description = KeyboardAndText.CUSTOM_CAPTION_UL_FILE
    ...
    if t_response:
        ...
        if file_size > Config.TG_MAX_FILE_SIZE:
            await update.message.edit_caption(
                caption=KeyboardAndText.RCHD_TG_API_LIMIT.format(
                    time_taken_for_download, convert_to_human_bytes(file_size)
                )
            )
        else:
            await update.message.edit_caption(
                caption=KeyboardAndText.UPLOAD_START
            )

            start_time = time.time()

            if tg_send_type == "video":
                width, height, duration = await video_metadata(download_directory)
                await update.message.reply_video(
                    ...
                    progress_args=(
                        KeyboardAndText.UPLOAD_START,
                        update.message,
                        start_time,
                    ),
                )
            elif tg_send_type == "audio":
                duration = await audio_metadata(download_directory)
                ...
                width, duration = await vm_video_metadata(download_directory)
            ...
            await update.message.edit_caption(
                caption=KeyboardAndText.AFTER_SUCCESSFUL_UPLOAD_MSG_WITH_TS.format(
